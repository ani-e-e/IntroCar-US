...
export function filterProducts({
  ...
  if (year) {
    ...
    if (!isNaN(yearNum)) {
      const skusForYear = new Set();
      Object.entries(fitmentLookup).forEach(([parentSku, fitments]) => {
        if (Array.isArray(fitments)) {
          // Check if any fitment covers this year
          const matchesFitment = fitments.some(f => {
            // If model filter is also active, only check that model's year range
            if (model && f.model !== model) return false;
            if (make && f.make !== make) return false;

            // Check chassis range for year
            // Most fitments have chassisStart/chassisEnd that can indicate year range
            // For now, we pass all if year is set (fitment data doesn't have direct year field)
            // The VehicleFinder already shows correct years from chassis data
            return true;
          });
          if (matchesFitment) {
            skusForYear.add(parentSku);
          }
        }
      });
      // Only filter if we have valid year data, otherwise keep products
      if (skusForYear.size > 0) {
        products = products.filter(p => skusForYear.has(p.parentSku));
      }
    }
  }
...
/**
 * Get all lookbooks with pagination
 */
export function getLookbooks({ page = 1, limit = 24, search = '' }) {
