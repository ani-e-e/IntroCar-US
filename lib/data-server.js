...
export function filterProducts({
  ...
  if (search) {
    ...
    // First check if the search term exists as a product SKU/parentSku
    const searchTermExistsAsProduct = products.some(p =>
      p.sku?.toUpperCase() === searchUpper ||
      p.parentSku?.toUpperCase() === searchUpper ||
      extractBaseSku(p.sku) === searchBase ||
      extractBaseSku(p.parentSku) === searchBase
    );

    // Check if this is an old part number (supersession) - but ONLY if no products match directly
    const exactSupersession = supersessionLookup[searchUpper];
    if (exactSupersession && !searchTermExistsAsProduct) {
      ...
      products = [...exactMatches, ...parentSkuMatches, ...startsWithMatches, ...skuMatches, ...supersessionContentMatches, ...descriptionMatches];
