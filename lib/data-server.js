...
export function filterProducts({
  ...
  // ============================================
  // ATTACH CHASSIS APPLICATION DATA TO PRODUCTS
  // This enables the product card hover to show fitment info
  // ============================================
  products = products.map(p => {
    const fitment = fitmentLookup[p.parentSku] || [];
    // Only include chassis application if we have real fitment data
    if (fitment.length > 0) {
      return {
        ...p,
        chassisApplication: fitment.map(f => ({
          make: f.make,
          model: f.model,
...
export function getSearchPartTypes() {
  ...
  for (const [value, count] of sortedTypes) {
    // Include all search part types including Lookbooks (shown as "Catalogues" in UI)
