...
export function filterProducts({
  ...
  // ============================================
  // COMPUTE AVAILABLE CATEGORIES (before category filter)
  // This ensures we only show categories that have products for current vehicle
  // ============================================
 ...
 */
export function getCatalogues({ page = 1, limit = 24, search = '', make = '', model = '', category = '', chassis = '' }) {
  ...
  catalogues = catalogues.filter(cat => cat.imageUrl && cat.imageUrl.trim() !== '');
  ...
  catalogues.forEach(cat => {
    ...
    if (cat.models) cat.models.forEach(m => allModels.add(m));
  ...
  if (make) {
    const modelsSet = new Set();
    catalogues.forEach(cat => {
      if (cat.models) cat.models.forEach(m => modelsSet.add(m));
    });
    modelsForMake = Array.from(modelsSet).sort();
  }
