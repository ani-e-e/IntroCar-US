 ...
 *
 * NOTE: These are fallback/default values. The primary source is now the
 * reseller_configs table in Supabase. Use getTenantAsync() for database fetch.
 */

import { createClient } from '@supabase/supabase-js';

// In-memory cache for tenant configs (5 minute TTL)
let tenantCache = {};
let cacheTimestamp = 0;
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

// Fallback tenant configurations (used if database is unavailable)
...
/**
 * Get Supabase client for tenant lookups
 */
...
/**
 * Transform database row to tenant config format
 */
function dbRowToTenant(row) {
  return {
    ...
    colors: row.colors || {},
    logo: row.logo_url,
    skuFilter: row.sku_filter,
    orderEmail: row.order_email,
    features: row.features || ['light'],
    showPrices: row.show_prices || false,
    showCart: row.show_cart || false,
...
/**
 * Get tenant configuration by slug (sync - uses cache or fallback)
 */
export function getTenant(slug) {
  // Check cache first
  if (tenantCache[slug] && Date.now() - cacheTimestamp < CACHE_TTL) {
    return tenantCache[slug];
  }
  // Fall back to hardcoded tenants
  return tenants[slug] || tenants['introcar-us'];
}

/**
 * Get tenant configuration by slug (async - fetches from database)
 * Use this in server components and API routes for most up-to-date data
 */
export async function getTenantAsync(slug) {
  // Check cache first
  if (tenantCache[slug] && Date.now() - cacheTimestamp < CACHE_TTL) {
    return tenantCache[slug];
  }
  ...
  if (supabase) {
    try {
      ...
      if (!error && data) {
        const tenant = dbRowToTenant(data);
        tenantCache[slug] = tenant;
        cacheTimestamp = Date.now();
        return tenant;
      }
    } catch (err) {
      console.error('Error fetching tenant from database:', err);
    }
  }
...
export function getTenantByDomain(domain) {
  // Check cache for domain match
  const cachedTenant = Object.values(tenantCache).find(t => t.domain === domain);
  if (cachedTenant && Date.now() - cacheTimestamp < CACHE_TTL) {
    return cachedTenant;
  }
...
/**
 * Get tenant by domain (async - fetches from database)
 */
export async function getTenantByDomainAsync(domain) {
  // Check cache first
  const cachedTenant = Object.values(tenantCache).find(t => t.domain === domain);
  if (cachedTenant && Date.now() - cacheTimestamp < CACHE_TTL) {
    return cachedTenant;
...
/**
 * Get all active tenants (async)
 */
export async function getAllTenantsAsync() {
  const supabase = getSupabase();
  ...
  if (supabase) {
    try {
      ...
      if (!error && data) {
        const tenantList = data.map(dbRowToTenant);
        // Update cache
...
/**
 * Get all tenant slugs (sync - from hardcoded + cache)
 */
export function getAllTenantSlugs() {
  const hardcodedSlugs = Object.keys(tenants);
  const cachedSlugs = Object.keys(tenantCache);
  return [...new Set([...hardcodedSlugs, ...cachedSlugs])];
}

/**
 * Get all tenant slugs (async - from database)
 */
export async function getAllTenantSlugsAsync() {
  const allTenants = await getAllTenantsAsync();
  return allTenants.map(t => t.slug);
}
